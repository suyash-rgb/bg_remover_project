  ...
  return (
    <div className="mb-16 relative">
          ...
          {Object.keys(categories).map((category) => (
            <button
              ...
              className={`px-6 py-2 rounded-full font-medium ${
                activeCategory === category ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-600 hover:bg-gray-200'
              }`}
      ...
      <div className="relative w-full max-w-4xl overflow-hidden m-auto rounded-xl shadow-lg">
        <img
          src={categories[activeCategory]?.with_bg}
          alt={`${activeCategory} category - original`}
          style={{ clipPath: `inset(0 ${100.2 - sliderPosition}% 0 0)` }}
        />

        <img
          src={categories[activeCategory]?.without_bg}
          alt={`${activeCategory} category - background removed`}
